...
REM ============================================================
REM 4. Minikube 상태 확인 및 시작 (수정됨!)
REM ============================================================
echo [🔧] Minikube 상태 확인 중...

REM 핵심 수정: call 사용하여 minikube 명령 실행
...
call kubectl cluster-info >nul 2>&1
if !errorlevel! neq 0 (
...
REM ============================================================
echo [🔍] ArgoCD 설치 확인 중...
call kubectl get namespace argocd >nul 2>&1
if !errorlevel! neq 0 (
    echo    → ArgoCD가 설치되지 않았습니다. 설치합니다...
    call kubectl create namespace argocd
    call kubectl apply -n argocd -f https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml
    echo    ⏳ ArgoCD Pod가 준비될 때까지 대기 중...
    call kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=argocd-server -n argocd --timeout=300s
    if !errorlevel! neq 0 (
...
call kubectl get namespace jenkins >nul 2>&1
if !errorlevel! neq 0 (
...
call kubectl get namespace dev >nul 2>&1
if !errorlevel! neq 0 (
    echo    → Dev 환경이 없습니다. 배포합니다...
    if exist "deploy\k8s\overlays\dev" (
        call kubectl apply -k deploy/k8s/overlays/dev
    ...
    call kubectl get pods -n dev 2>nul
)
echo.
...
call kubectl get svc -n jenkins jenkins >nul 2>&1
if !errorlevel! equ 0 (
    echo    → Jenkins (8090)
